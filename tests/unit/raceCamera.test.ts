...
import { CAMERA_MODES, getRaceCameraPose, nextCameraMode } from '../../src/render/RaceCamera';
import type { PhysicsSnapshot } from '../../src/sim/types';

describe('race camera', () => {
  it('removes chase and cycles only between onboard and nose', () => {
    expect(CAMERA_MODES).toEqual(['onboard', 'nose']);
    expect(nextCameraMode('onboard')).toBe('nose');
    expect(nextCameraMode('nose')).toBe('onboard');
  });
